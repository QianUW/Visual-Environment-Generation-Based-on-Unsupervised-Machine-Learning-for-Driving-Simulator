...
class UMLDS(object) :
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.epoch):#每个epoch保存一次，也就是训练iteration次保存一次；另外每save_freq步保存一次
            ...
            for idx in range(start_batch_id, self.iteration):
                try:
                    ...
                    batch_A_images, batch_B_images, fake_A, fake_B, _, g_loss, summary_str = self.sess.run(
                        [self.real_A, self.real_B, self.fake_A, self.fake_B, self.G_optim, self.Generator_loss,
                         self.G_loss], feed_dict=train_feed_dict)
                    self.writer.add_summary(summary_str, counter)
    ...
    def load(self, checkpoint_dir):
        ...
        checkpoint_dir = os.path.join(checkpoint_dir, self.model_dir)#model_dir为（muunit_dataset_name_lsgan）

        ckpt = tf.train.get_checkpoint_state(checkpoint_dir)
        if ckpt and ckpt.model_checkpoint_path:#返回最新checkpoint文件
    ...
    def test(self):
        tf.global_variables_initializer().run()
        test_A_files = glob('./dataset/{}/*.*'.format(self.dataset_name + '/testA'))#得到testA文件夹下图片名称list
        test_B_files = glob('./dataset/{}/*.*'.format(self.dataset_name + '/testB'))

        self.saver = tf.train.Saver()
        could_load, checkpoint_counter = self.load(self.checkpoint_dir)#check_point dir默认为checkpoint;load加载权值
        self.result_dir = os.path.join(self.result_dir, self.model_dir)
        check_folder(self.result_dir)#没有则创建文件夹
        ...
        for sample_file  in test_A_files : # A -> B
            ...
            for i in range(self.num_style) :#num_style为要输出图片数
                test_style = np.random.normal(loc=0.0, scale=1.0, size=[1, 1, 1, self.style_dim])#随机生成符合正太分布的s向量，style_dim默认为8
                image_path = os.path.join(self.result_dir, '{}_style{}.{}'.format(file_name, i, file_extension))
                print(file_name,str(i),test_style)
    ...
    def spc_test(self):
        ...
        for sample_file in test_A_files:  # A -> B
            ...
            for i in range(self.num_style):  # num_style为要输出图片数
                #test_style = np.array([[[[0.42499207, -2.80052888, -1.27175939, 0.38885975, -1.26436245, 2.47716331,-0.02827566, 0.12728485]]]])  # 随机生成符合正太分布的s向量，style_dim默认为8
                #test_style = np.array([[[[ 0.59932866, -0.63002128, 0.07587199, -0.07933834, -1.2467873, 0.54258754, -0.82982448, -0.66341979]]]])#入夜
                #test_style = np.array([[[[-0.7843677,  -1.73333418, -0.59294801,  0.3364004,  -0.50422754, -0.55436861, -0.74849017,  1.44244231]]]])#深夜
                test_style = np.array([[[[-0.00501374, 2.08136842, -0.03810636, -0.19362248, -1.03384617, -0.51903289, 0.53973022, 0.25999474]]]])  #路灯
                image_path = os.path.join(self.result_dir, '{}_style{}.{}'.format(file_name, i, file_extension))
                print(file_name, str(i), test_style)
                fake_img = self.sess.run(self.test_fake_B,
                                         feed_dict={self.test_image: sample_image, self.test_style: test_style})
                save_images(fake_img, [1, 1], image_path)
